...
只要 URL 直接指向 `/v1/chat/completions` endpoint，也可以使用第三方 OpenAI 兼容提供商。点击 **Test connection** 发送真实完成请求（"ping"）并验证返回非空内容，报告状态、结束原因和内容预览——输入 API key 后也会自动测试连接。
...
撤销最近应用的变更计划——删除创建的文件，恢复修改的文件，重新创建被删除的文件。确认通知会显示计划摘要和操作数量。历史记录保留最近 20 条变更。
